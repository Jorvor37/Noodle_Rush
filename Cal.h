...
// Functions
void CashierSystem::addMenuItem(string name, float price) 
    {
        /*
...
void CashierSystem::checkout() 
    {
        ...
        */
        ...
        float payment = selectedItem->price + ((rand() % 100 + 1)); // Random payment larger than menu price
        float correctChange = payment - selectedItem->price;
        float playerChange;
        float finalChange;
        ...
        if (playerChange == correctChange) {
            float tip = 0.3 * selectedItem->price;
